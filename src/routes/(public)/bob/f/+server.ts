...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	if (isNaN(offset) || isNaN(limit)) return error(400, "Incorrect offset or limit");
	if (limit < 0) limit = 0;
	...
	if (locals.user && !url.searchParams.get("public")) q.where(and(eq(bobImage.userId, locals.user.id), isNull(bobImage.deletedAt)));
